...
#include "pch.h"
#include "SocialCreditTestGame.xaml.h"
#include <winrt/Windows.Foundation.h>
#include <winrt/Microsoft.UI.Xaml.Media.Imaging.h> // for uri source
#include <fstream>
#if __has_include("SocialCreditTestGame.g.cpp")
#include "SocialCreditTestGame.g.cpp"
#endif
...
{
    SocialCreditTestGame::SocialCreditTestGame()
    {
        InitializeComponent();
    }

    int32_t SocialCreditTestGame::MyProperty()
    {
        throw hresult_not_implemented();
    }

    void SocialCreditTestGame::MyProperty(int32_t /* value */)
    {
        throw hresult_not_implemented();
    }
...
Windows::Foundation::IAsyncAction winrt::Terrible_Programs_Installer::implementation::SocialCreditTestGame::PopulateAssets()
{
...
#pragma region Downloader
    ...
    std::list<std::wstring> list = downloader.A_GetSCTGAssets();
...
#pragma region AssetStuff
    ...
    */
    ...
    DebugTools::Downloader::PrintList(stg, L"SCTG assets Downloaded");
    int x = 0;
    for (std::wstring item : stg)
    {
        if (x == 0) {
            ...
            std::ifstream descfile(DebugTools::Downloader::AssetLocation + L"\\SCTGDESC.txt");
            if (descfile.is_open()) {
...
void winrt::Terrible_Programs_Installer::implementation::SocialCreditTestGame::Page_Loaded(winrt::Windows::Foundation::IInspectable const& sender, winrt::Microsoft::UI::Xaml::RoutedEventArgs const& e)
{
    //Cache checks
    if (DebugTools::Downloader::_VerifySCTGCache()) {
        //Desc
        std::wstring text;
        std::string line;
        std::ifstream descfile(DebugTools::Downloader::AssetLocation + L"\\SCTGDESC.txt");
        ...
        int x = 0;
        for (std::wstring item : DebugTools::Downloader::SCTGItems)
        {
            if (x == 0) {
                x++;
                continue;
...
void winrt::Terrible_Programs_Installer::implementation::SocialCreditTestGame::Button_Click(winrt::Windows::Foundation::IInspectable const& sender, winrt::Microsoft::UI::Xaml::RoutedEventArgs const& e)
{
